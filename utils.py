...
def replace_custom_layer(custom_layer, layers, i, replace_with=None):
            if replace_with is not None:
                layer = copy.copy(replace_with)
            else:
                layer = copy.copy(custom_layer)
            print(f"{layers[i]} at index {i} replaced with {custom_layer}")
            layer.set_index(i)
            layers[i] = layer

def insert_custom_layer(custom_layer, layers, i):
    print(f"{custom_layer} inserted at index {i}")
    layer = copy.copy(custom_layer)
    layer.set_index(i)
